  ...
  const toggleMobileBulkActions = () => {
    setMobileActionsOpen((open) => {
      ...
      if (nextOpen && window.innerWidth < 768) {
        // Ao abrir no mobile, levamos a viewport até os botões expandidos sem torná-los sticky.
          ...
          <section className="cx-scroll-panel">
            ...
            {showBulkToolbar ? (
              <>
                ...
                <div
                  data-testid="category-bulk-actions-panel"
                  className={`${mobileActionsOpen ? "block" : "hidden"} cx-scroll-panel-toolbar md:block`}
                >
            ...
            {orderedCategories.length === 0 ? (
              ...
              <div className="space-y-2 sm:space-y-0">
                ...
                {orderedCategories.length <= 2 ? (
                  <div className="rounded-2xl border border-amber-200 bg-[#fff7e6] px-4 py-3 text-xs leading-5 text-zinc-700 sm:mt-3">
                    Organize as categorias como blocos do cardápio público. Uma boa ordem ajuda o cliente a encontrar
                    destaques, pratos principais e acompanhamentos mais rápido.
                  </div>
                ) : null}
              </div>
            )}
